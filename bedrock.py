...
class CohereEmbeddingsModel(BedrockEmbeddingsModel):
    ...
    def _parse_args(self, embeddings_request: EmbeddingsRequest) -> dict:
        ...
        """
            ...
            # Handle iterable input without tiktoken
            for inner in embeddings_request.input:
                if isinstance(inner, (str, int)):
                    ...
                    try:
                        # Join the sequence elements if they're strings/numbers
                        text = " ".join(str(item) for item in inner)
                        texts.append(text)
                    except Exception as e:
                        logger.warning(f"Could not process input: {str(e)}")
                        continue
    ...
    def embed(self, embeddings_request: EmbeddingsRequest) -> EmbeddingsResponse:
        ...
        return self._create_response(
            ...
            model=embeddings_request.model,
            encoding_format=embeddings_request.encoding_format,
            input_tokens=response_body.get("inputTextTokenCount", 0)  # Added this line
        )
